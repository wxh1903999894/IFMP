        ...
        public void MyIntegral(HttpContext context)
        {
            try
            {
                ...
                using (IFMPDBContext db = new IFMPDBContext())
                {
                    ...
                    var dormit = db.Dormitory.ToList();
                    var isin = dormit.Count;
                    int i = 0;
                    foreach (var dor in dormit)
                    {   
                        if (!dor.DormiUser.Split(',').Any(x => x == uid))
                        {
                            i = i + 1;
                        }
                        if (isin == i)
                    ...
                    //年度
                    int yearscore = 0;
                    List<Dormitory> DormitoryListYear = db.Dormitory.Where(x => x.CreateDate > beginYear && x.CreateDate < EndYear).ToList();
                    foreach (Dormitory dormitory in DormitoryListYear)
                    {
                        if (dormitory.DormiUser.Split(',').Any(x => x == uid))
                        {
                            whatdormitory = dormitory.DormiName;
                            List<SpotCheck> model = db.SpotCheck.Where(x => x.DormitoryId == dormitory.ID && x.CreateDate >= beginYear && x.CreateDate < EndYear).ToList();
                    ...
                    //月度
                    int monthscore = 0;
                    List<Dormitory> DormitoryListMonth = db.Dormitory.Where(x => x.CreateDate > BeginMonth && x.CreateDate < EndMonth).ToList();
                    foreach (Dormitory dormitory in DormitoryListMonth)
                    {
                        if (dormitory.DormiUser.Split(',').Any(x => x == uid))
                        {
                            List<SpotCheck> model = db.SpotCheck.Where(x => x.DormitoryId == dormitory.ID && x.CreateDate >= BeginMonth && x.CreateDate < EndMonth).ToList();
                            if (model.Count > 1)
                            {
                                int score = 0;
                                foreach (SpotCheck spotcheckmodel in model)

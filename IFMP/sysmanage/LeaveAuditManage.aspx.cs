...
{
    public partial class LeaveAuditManage : PageBase
    {
        ...
        private void DataBindList()
        {
            ...
            {
                List<LeaveAudit> LeaveAuditList = db.LeaveAudit.Where(t => t.UserID == UserID && (LeaveState == -2 || t.LeaveState == (LeaveState)LeaveState)).ToList();
                var list = from leaveaudit in db.LeaveAudit
                           join leave in db.Leave on leaveaudit.LeaveID equals leave.ID
                           join user in db.User on leave.UserID equals user.ID
                           where leaveaudit.UserID == UserID
                           && (LeaveState == -2 || leaveaudit.LeaveState == (LeaveState)LeaveState)
                           && (LeaveType == -2 || leave.LeaveType == (LeaveType)LeaveType)
                           && leave.BeginDate >= begindate
                           && leave.BeginDate <= enddate
                           && leave.IsDel != true
                           && user.IsDel != true
                           //&& leaveaudit.SendDate >= begindate
                           //&& leaveaudit.SendDate <= enddate
                           orderby leaveaudit.SendDate descending
                           select new
                                  {
                                      LID = leave.ID,
                                      user.RealName,
                                      ...
                                      leave.Day,
                                      leave.CreateDate,
                                      leaveaudit.AuditDate,
                                      leaveaudit.SendDate,
                                      leaveaudit.LeaveState
                                  };
                ...
                this.rp_List.DataSource = list.OrderByDescending(t => t.SendDate).Skip((Pager.CurrentPageIndex - 1) * Pager.PageSize).Take(Pager.PageSize).ToList(); ;

...
namespace IFMP.taskflow
{
    public partial class MyTaskDetail : PageBase
    {
        ...
        {
            Task model = db.Task.FirstOrDefault(t => t.ID == TaskID);
            //Flow flow = db.Flow.FirstOrDefault(t => t.ID == db.TaskFlow.FirstOrDefault(m => m.ID == TaskFlowID).FlowID);
            //List<Flow> flowlist = db.Flow.Where(t => t.TableTypeID == tabtype && t.ParentID <= flow.ParentID).OrderBy(t => t.ParentID).ToList();

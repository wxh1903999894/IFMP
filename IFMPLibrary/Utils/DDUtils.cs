...
namespace IFMPLibrary.Utils
{
    public class DDUtils
    {
        ...
        public string GetAPPAccessToken(string key, string secret)
        {
            string accesstokendate = new BaseUtils().GetXML("Setting/DDSetting/AppAccessTokenDate");

            if (string.IsNullOrEmpty(accesstokendate) || Convert.ToDateTime(accesstokendate) < DateTime.Now)
            {
                string accessget = GetData(string.Format("https://oapi.dingtalk.com/gettoken?appkey={0}&appsecret={1}", key, secret));
                ...
                new BaseUtils().SetXML("Setting/DDSetting/AppAccessToken", jo["access_token"].ToString());
                new BaseUtils().SetXML("Setting/DDSetting/AppAccessTokenDate", DateTime.Now.AddSeconds(expires * 0.8).ToString("yyyy-MM-dd HH:mm:ss"));
            }

            string accesstoken = new BaseUtils().GetXML("Setting/DDSetting/AppAccessToken");
            return accesstoken;
        }
        ...
        public string GetAccessToken()
        {
            ...
            {
                FileStream fs = new FileStream(@"d:\IFMP.txt", FileMode.OpenOrCreate, FileAccess.Write);
                StreamWriter sw = new StreamWriter(fs);
                sw.BaseStream.Seek(0, SeekOrigin.End);
                sw.WriteLine("当前时间:" + ex.Message);
                sw.Flush();
                sw.Close();
                fs.Close();
            }
        ...
        {
            string accesstoken = GetAPPAccessToken(ParaUtils.DormitroyKey, ParaUtils.DormitroySecret);
            string accessget = GetData(string.Format("https://oapi.dingtalk.com/user/getuserinfo?access_token={0}&code={1}", accesstoken, code));
            JObject jo = (JObject)JsonConvert.DeserializeObject(accessget);
            if (jo["userid"] != null)
            ...
            //    String canonicalString = DingTalkSignatureUtils.GetCanonicalStringForIsv(dingTimestamp, null);
            //    String signature = DingTalkSignatureUtils.ComputeSignature(ParaUtils.LoginAppSecret, canonicalString);
            //    signature = System.Web.HttpUtility.UrlEncode(signature, System.Text.Encoding.UTF8);
            //    signature = signature.Replace("+", "%20").Replace("*", "%2A")
            //              .Replace("~", "%7E").Replace("/", "%2F");


            //    String testcanonicalString = DingTalkSignatureUtils.GetCanonicalStringForIsv(1546084445901, null);
            //    String testsing = DingTalkSignatureUtils.ComputeSignature("testappSecret", testcanonicalString);
            //    testsing = System.Web.HttpUtility.UrlEncode(testsing, System.Text.Encoding.UTF8);
            //    testsing = testsing.Replace("+", "%20").Replace("*", "%2A")
            //              .Replace("~", "%7E").Replace("/", "%2F");



            //    JObject GetData = new JObject();
            //    GetData.Add("tmp_auth_code", code);
            //    string url = string.Format("https://oapi.dingtalk.com/sns/getuserinfo_bycode?signature={0}&timestamp={1}&accessKey={2}", signature, dingTimestamp + "", ParaUtils.LoginAppID);
            //    JObject ReturnData = PostData(url, GetData);
